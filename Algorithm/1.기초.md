...
>    \end{cases}
>$$
>이므로 $T(n) = \Theta(nlog_2n)$이다.  
>따라서 병합정렬$\Theta(nlog_)$이 삽입정렬$\Theta(n^2)$보다 성능이 좋다.
...
\Theta(g(n))=\{
        f(n):
        \exists c_1,c_2,n_0 >0 \quad s.t \quad \forall n\geq n_0,\\
...
O(g(n))=\{
        f(n):
        \exists c_2,n_0 >0 \quad s.t \quad \forall n\geq n_0,\\
...
O(g(n))=\{
        f(n):
        \exists c_1,n_0 >0 \quad s.t \quad \forall n\geq n_0,\\
...
o(g(n))=\{
        f(n):
        \exists n_0 >0 \quad s.t \quad \forall n\geq n_0,\forall c_2 > 0,\\
        0 \leq f(n) < c_2g(n)
    \}\\
...
\omega(g(n))=\{
        f(n):
        \exists n_0 >0 \quad s.t \quad \forall n\geq n_0,\forall c_1 > 0,\\
        0 \leq c_1g(n) < f(n)
    \}\\
